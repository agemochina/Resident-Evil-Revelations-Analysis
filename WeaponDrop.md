...
- 寻宝插件(Rare Finders/人品插件)，会对蓝底几率产生巨大的影响。
- 想刷到好东西，正确方法带好寻宝插件，拼箱子数。怎么快怎么来，弄其他玄学都没用。箱子开的越多理论上开出好东西概率越高。
...
- 带9寻宝，蓝底+异名+满孔 = 16.4% * 1.5% * 0.4%  ≈ 0.1% 约等于千分之1

换句话说：
- 无论7、8、9个寻宝插件，跑7武器平均每局蓝底1.1个（不是说必出，是大数据统计），跑9武器平均每局蓝底1.4个
- 平均开1000个箱子能出1个蓝底满孔异名
...
```
代码解释：先打分，即随机产生一个 [0.0, 100.0) 之间的小数，然后查表获得标签。只要这个数字确定了，标签就确定。
...
解释：无标签为 50%，异名（0x8009000B） 为1.5%，可以看出，只有随机分超过98.5分才是异名
