...
----------------------------------------
```
上述文件就是各种类型武器的掉率，物品池所有的几率加在一起是100%，每次掉落时从这个池子里根据固定几率随机挑。
例如 0x80051100, 5.0, 0 这行意思是：0x80051100这个武器代号(指Python) 掉率5.0%


# 备注：关于表格字段名(TITLE)
用工具解析时，发现field_titles=['', '', ''] 是空的，这是因为可能是官方为了保密？游戏打包的时候把表格的TITLE（字段名）删除了，但还有很多表格字段名还在，例如 BPTable 等。
